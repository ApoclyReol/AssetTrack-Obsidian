  ...
  return groups.length === 0
    ? <EmptyState text={overview
      ? groupBy === "counterparty"
        ? t("暂无交易对手记录。", "No counterparty records yet.")
    ...
    : <div className="asset-track-table-scroll asset-track-history-group-scroll"><table className={`asset-track-history-group-table${overview ? " asset-track-history-group-table--overview" : " asset-track-history-group-table--health"}`}>
      <thead><tr>
        ...
        <th scope="col"><HistorySortButton field="product" label={groupBy === "product" ? t("商品", "Item") : t("交易对手", "Counterparty")} sort={sort} onSort={onSort} /></th>
...
export function ProductOverviewTable(props: ProductHistoryTableProps & { groupBy?: "product" | "counterparty" }) {
  return <ProductGroupTable {...props} overview groupBy={props.groupBy} />;
}
