...
export function IssueList({
  ...
  const summary = blocking > 0
    ...
    : omittedCount > 0
      ? t(
        `共 ${issues.length} 项保存后的提醒，按优先级显示前 ${visibleIssues.length} 项：`,
        `${issues.length} saved warnings found. Showing the first ${visibleIssues.length} by priority:`
      )
      : t(
        "以下为保存后的提醒，已按优先级排序：",
        "The following saved warnings are sorted by priority:"
      );
  return (
