...
v1.3.0 代码候选已完成纯 TypeScript 架构、个性化金额设置、双资产口径、周期消费面板、自动中英文界面与规则工作台：
...
4. 完成后再提交、推送 `1.3.0` 标签并核验 GitHub Release 三文件与 attestations。

## 后续考虑

- 将 `AssetTrackEditorApp.tsx` 按分析、月度编辑、流水、导入、规则和共享组件逐步
  拆入 `features/`；拆分时保持 Service/Repository 接口和用户行为不变。
- 将 CSV/XLSX/XLS 解析真正迁入 Worker；v1.3.0 仍只完成 `ArrayBuffer` 读取并移除
