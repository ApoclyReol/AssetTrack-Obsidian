...
> 文档角色：开发与维护。本文定义公式、数据语义和测试口径，不是面向用户的财务教程。

项目的核心价值在于其财务逻辑的严密性。

## 0. 唯一事实层与数据质量闭环

SQLite 是唯一持久化财务事实源。账户、流水、借款、理财、固定资产、分类和规则在
统一 schema 与规范语义中保存；`data.json` 只保存插件设置和导入映射元数据，不承载
财务事实。账单文件是原始输入，React 草稿是待提交状态，图表与商品汇总是查询结果，
导出与报告是输出物，它们都不能成为第二套账本。

所有月度、年度、商品、分类、资产和对账指标必须复用同一套领域定义与计算实现。
分析数字应尽可能能回到分类、商品、逐笔流水以及输入来源；如果结果无法解释来源，
它只能作为待完善的展示，不应直接成为新的事实。

资产闭环是数据质量门禁：月末资产状态、借款、理财变动和流水共同验证资金变化是否
闭合。对账差额不是“看起来异常”的装饰指标，而是发现漏导账单、漏记收入、错误类型、
资产快照偏差和特殊资金变动遗漏的排查入口。
...
- 单笔支出金额达到用户设置的“大额支出阈值”（默认 1000）。

### 4.4 借款余额 `AssetTrackRepository.activeDebt()`

借款记录按月份动态生效，不写入月度快照表。月流水页中的借款区块只是
`debt_manager` 的月度投影：本月底仍未还、当月新增、当月还清的记录会出现在
当前月份；保存月份时仍更新同一张事实表。
...
- 月流水页勾选“本月还清”时，系统把 `paid_date` 写为该月月末；当月新增又当月还清
  的借款会保留为已还清事实记录，但不改变当月月末借款余额。
- 如果一笔借款已经在未来月份标记还清，查看更早月份时它只读显示为当月未还；不能从
  更早月份提前还清、改金额或删除，Repository 会拒绝这类写入。
...
market_net_assets = cash - debt + market_value + investment_cash
total_assets = cost_assets  # 兼容字段
```

- 对账、理论消费和储蓄率继续使用资金投入口径，不受市场价格波动影响。
- 财富趋势同时展示市场净资产与资金投入资产，固定资产不进入任一口径。
- 月流水中的借款区块只编辑借款事实；上月未还自动继承，当月新增或勾选本月还清会改变
  当月债务余额，并实时进入对账差额。
- 平账容差默认 100 元，大额支出阈值默认 1000 元，均以插件设置为事实来源。
- 周期消费按最近 12 个有数据月份、周期分类、商品名称汇总，只描述历史事实。
