...
本文补充 v1.6.0 当前架构中与导入容错、月流水警告和配置中心有关的接口边界。它不改变
...
界面首次进入使用轻量的 `ruleWorkspaceShell()`，再按当前页面需要加载 `ruleWorkspaceAnalytics()`。
...
规则解析统一由 `src/domain/rules.ts` 提供，按收支类型和规范化商品建立唯一匹配键。
相同键命中多个不同分类时，兼容诊断会返回冲突并让 `applyRulesWithIssues()` 保留原分类；
正常保存路径会阻止同一键重复写入。月流水和历史详情使用同一个解析结果，因此规则解释和规则 ID
不会因页面不同而产生分歧。

交易对方不属于规则条件，只在流水与历史详情中显示；用户在规则页面只维护收支类型、商品和分类。

`productHistory(query)` 返回稳定的数据库流水 ID、月份、日期、交易对方、商品、原分类、
分类启用状态、金额和规则解释。商品统计按“收支类型 + 规范化商品”聚合，商品-分类冲突按
商品级审计。多数分类只作为建议，不自动改写历史。

规则审计状态与实际覆盖范围仍由后端兼容接口独立计算。当前数据健康不把“无规则”、重复规则
或规则覆盖作为独立问题类型；规则建议只根据未覆盖且分类稳定的流水生成。
...
后才能确认写入；商品总览提供规则创建入口，不提供“打开最近月份”导航入口。
