...
插件不依赖 Python、sidecar、平台原生扩展或 CPU 架构包。v1.8.1 已完成自动化构建、
跨平台 CI、标准三文件发布和 attestation；真实 Obsidian smoke 与复制 Vault 回归
仍按发布日志作为发布后的人工质量门禁持续补充。Linux 验证属于发布后持续质量工作。

## 稳定兼容边界

- `manifest.json` 最低 Obsidian 版本为 1.13.0；安装或更新 v1.8.1 前要求用户先升级
  到当前最新的 1.13.x 桌面版。
- 运行时探测 Node ≥22.16、`DatabaseSync` 和 `sqlite.backup`；旧桌面安装器只显示
  升级提示，不修改数据。
- `versions.json` 声明当前版本最低兼容 Obsidian 版本。
- 当前开发线使用 schema 11；schema 9/10 首次打开时先创建保护备份并按版本链迁移，迁移失败
  不覆盖原数据库。完整迁移链在同一事务中提交。
- 显示设置、AI 地址/模型/超时保存在插件 `data.json`，API Key 使用 SecretStorage；分类描述、通用规则、代付规则类型、流水来源、
  理财流水账户和操作日志进入 schema 11 备份，规则工作台的历史统计只读取已保存月份。
...
6. v1.8.1 发布后持续记录复制 Vault 升级、中文/英文界面、商品-分类冲突与回溯事务、商品
