...
插件不依赖 Python、sidecar、平台原生扩展或 CPU 架构包。v1.8.0 已完成自动化构建、
跨平台 CI、标准三文件发布和 attestation；真实 Obsidian smoke 与复制 Vault 回归
仍按发布日志作为发布后的人工质量门禁持续补充。Linux 验证属于发布后持续质量工作。

## 稳定兼容边界

- `manifest.json` 最低 Obsidian 版本为 1.13.0；安装或更新 v1.8.0 前要求用户先升级
  到当前最新的 1.13.x 桌面版。
...
6. v1.8.0 发布后持续记录复制 Vault 升级、中文/英文界面、商品-分类冲突与回溯事务、商品
