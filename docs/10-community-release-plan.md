...
插件不依赖 Python、sidecar、平台原生扩展或 CPU 架构包。**三平台真实 Obsidian
smoke 仍是提交前发布门槛；未实测的平台不得标记完成。** Community Directory
中的插件 ID `asset-track` 和名称 `Asset Track` 也必须在提交当天再次搜索确认。
...
| 全新安装和启用 | 未测试 | 未测试 | 未测试 |
| 创建新数据库 | 未测试 | 未测试 | 未测试 |
| 载入已有 schema 9 数据库 | 未测试 | 未测试 | 未测试 |
| CSV 导入 | 未测试 | 未测试 | 未测试 |
| XLSX / XLS 导入 | 未测试 | 未测试 | 未测试 |
| 保护快照、手动 ZIP 与恢复 | 未测试 | 未测试 | 未测试 |
