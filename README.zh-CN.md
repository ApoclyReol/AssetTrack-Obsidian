...
说明、交易对方和交易状态。缺少日期或空日期会默认到所选月份 1 日，空状态可以明确
选择是否导入；只有警告的流水可以先保存，解析与规则匹配全部在本地完成。

规则工作台提供健康摘要、商品级历史、规则解释、商品名称统一、分类回溯预览和带
revision 保护的批量修改。分类、匹配规则和历史修正分别保存，并分别报告结果。规则
实际覆盖范围与重复、冲突状态分开显示，部分覆盖商品仍可针对未覆盖流水创建建议。

商品名称统一默认从当前收支类型和主要分类开始，可切换分类或搜索其他商品组。关闭
带未保存月流水、借款、分类或规则的视图时，如果取消放弃，插件会在当前会话中重新
打开并恢复内存草稿；重启应用或插件后不保证恢复。
